...
This Non-disclosure Agreement (the "\begin{bf}Agreement\end{bf}") is made and entered into as of the
\BLOCK{ if isEffectiveDateSpecific }
\VAR{contractDated}
\BLOCK{ else }
last date signed below
\BLOCK{ endif }
(the  "\begin{bf}Effective Date\end{bf}") by and between \VAR{ownerName},
\BLOCK{ if isOwnerCompany }
a \VAR{ ownerState } corporation with offices at
\BLOCK{ else }
an individual residing at
\BLOCK{ endif }
\VAR{ ownerAddress }, \VAR{ ownerCity }, \VAR{ ownerStateInitials} \VAR{ ownerZipCode } (the "\begin{bf}Owner\end{bf}"), and
\VAR{recipientName},
...
an individual residing at
\BLOCK{ endif }
\VAR{ recipientAddress }, \VAR{ recipientCity }, \VAR{recipientStateInitials} \VAR{ recipientZipCode } (the "\begin{bf}Second Party\end{bf}").
